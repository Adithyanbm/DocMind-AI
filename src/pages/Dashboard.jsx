...
import {
  ...
  Mic, MicOff, Star, Pencil, Trash2, Layers, Compass, MessageCircle
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        setAttachedFile({ name: file.name, type: file.type, size: file.size, base64: base64String });
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.type.indexOf('image') !== -1) {
        ...
        if (file) {
          ...
          } catch (err) { console.error("Failed to convert pasted image to base64", err); }
        }
        break;
      }
    }
  ...
  const handleSubmit = async (e, options = { fromVoice: false, retryIndex: null }) => {
    ...
    if (options.retryIndex !== null && options.retryIndex !== undefined) {
      const uIdx = options.retryIndex - 1;
      if (uIdx < 0 || messages[uIdx].role !== 'user') return;
      messagesToMap = messages.slice(0, uIdx + 1);
      ...
      displayUserMessage = {
        role: 'user', content: input, attachment: attachedFile ? attachedFile.name : null,
    ...
    try {
      ...
      if (!startingFileId) startingFileId = await autoSaveToDrive([...apiMessages], undefined, streamTrackerId);
      ...
      const finalAssistantMessage = { role: 'assistant', content: assistantContent, reasoning_content: assistantReasoning };
      autoSaveToDrive([...apiMessages, finalAssistantMessage], startingFileId, streamTrackerId);
  ...
  useEffect(() => { 
    ...
    return () => { delete window.dashboardHandleRetry; };
  }, [handleSubmit]);
              ...
              <div className="input-container-wrapper" style={{ position: 'relative' }}>
                {showTemplateMenu && messages.length === 0 && (
                  <div className="template-menu template-popup">
                    <div className="template-header">
                      <div className="template-header-left"><span className="code-tag">&lt;/&gt; Code</span></div>
                      <button className="icon-btn" style={{ width: 20, height: 20 }} onClick={() => setShowTemplateMenu(false)}><X size={14} /></button>
                    </div>
                    <div className="template-list">
                      {["Develop algorithm solutions", "Tell me what programming paradigm suits my thinking style", "Help me turn a screenshot into working code", "Develop deployment strategies"].map(t => (
                        <button key={t} className="template-item" onClick={() => { setInput(t); setShowTemplateMenu(false); }}>{t}</button>
                      ))}
                      <button className="template-item" onClick={() => { setInput("Design scalability plans"); setShowTemplateMenu(false); }}>
                        <span>Design scalability plans</span><ChevronDown size={16} style={{ transform: 'rotate(-90deg)' }} />
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  {attachedFile && (
                    <div className={attachedFile.type.startsWith('image/') ? "attachment-image-preview" : "attachment-chip"}>
                      {attachedFile.type.startsWith('image/') ? (
                        ...
                        <><span className="file-name">{attachedFile.name}</span><button onClick={removeAttachment}><X size={14} /></button></>
                      )}
                    </div>
                  )}

                  <textarea
                    ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} onPaste={handlePaste}
                    placeholder="How can I help you today?" disabled={isStreaming} rows={1} className="chat-textarea"
                  />
                    ...
                    <div className="input-controls-right">
                      ...
                      {(isListening || isStreaming || isAiSpeaking) ? (
                        ...
                        <button
                          ...
                          onClick={(e) => { if (input.trim() || attachedFile) handleSubmit(e); else toggleListening(); }}
                ...
                {messages.length === 0 && (
                  <div className="suggestion-tags">
                    <button className="tag" onClick={() => setShowTemplateMenu(!showTemplateMenu)}><span className="code-tag" style={{ border: 'none', padding: 0, background: 'transparent', fontSize: '0.85rem' }}>&lt;/&gt;</span> Code</button>
                    <button className="tag"><MessageSquare size={14} /> Summarize notes</button>
                    <button className="tag"><HardDrive size={14} /> Analyze architecture</button>

...
def chat_completions(request):
    try:
        ...
        formatting_sys_prompt = {
            "role": "system", 
            "content": (
                "You are DocMind AI, a helpful assistant. Respond naturally to greetings and general questions.\n\n"
                "ONLY IF the user requests a project, a coding task, or a set of files, follow this precise flow:\n\n"
                ...
                "### Project Requirements (ONLY for coding/project tasks):\n"
                "Generate the following as independent code blocks if applicable:\n"
                "1. **setup_script.sh**: Commands to scaffold the project\n"
                "2. **Dependency file**: (e.g., `package.json`, `pubspec.yaml`, `requirements.txt`)\n"
                "3. **Models**: All data models, interfaces, types, or classes\n"
                "4. **Theme/Design System**: Colors, typography, spacing\n"
                "5. **Seed Data**: (if applicable) with rich content\n"
                "6. **Additional files**: Screens, components, services, etc.\n\n"
                "### Quality Rules for Code/Data:\n"
                "- explanations must be 2-4 sentences, genuinely educational\n"
                "- funFacts must be surprising, specific, and verifiable\n"
                "- Every file must be complete and runnable â€” no placeholders\n"
                "- Include all imports and add brief inline comments\n\n"
                "### Format Rules:\n"
